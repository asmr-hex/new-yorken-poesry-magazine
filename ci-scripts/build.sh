...
# package up bundles frontend, docker-compose.prod.yml, .env,
# blackbox, secrets (encrypted), migrations into directory
mkdir prod-deploy
mv ./client/build/* ./prod-deploy
mv docker-compose.prod.yml ./prod-deploy/docker-compose.yml
mv .env ./prod-deploy/.env
mv .blackbox ./prod-deploy/.blackbox
mv .secrets.gpg ./prod-deploy/.secrets.gpg
